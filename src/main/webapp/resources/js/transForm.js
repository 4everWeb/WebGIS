	...
	foodLayer = new ol.layer.Vector(
			{
				source : new ol.source.Vector(
						{
							url : ServerUrl
									+ "map/wfs?SERVICE=WFS&REQUEST=GetFeature&TYPENAME=tilemaker:cjw_food&VERSION=1.0.0&outputFormat=application/json&cql_filter=DWITHIN(geom,POINT("
									+ String(transXY[0]) + " "
									+ String(transXY[1]) + "),2000,meters)",
...
function markUserDestination() {
	...
	for (i = 0; i < addressXY.length; i++) {
		window["iconFeature" + i] = new ol.Feature({
			geometry : new ol.geom.Point([ Number(addressXY[i].X),
					Number(addressXY[i].Y), ]),
		});
	...
	if (Object.keys(centerXY).length == 0) {
		iconFeatureCenter = new ol.Feature({
			geometry : new ol.geom.Point([
					Number(getCenterOfCoord(centerFeature.getGeometry()
							.getCoordinates())[0]),
					Number(getCenterOfCoord(centerFeature.getGeometry()
							.getCoordinates())[1]), ]),
		});
...
function destinationCenterMove() {
	...
	case 2:
		...
		map.getView().fit([ arrX[0], arrY[0], arrX[1], arrY[1] ], {
			size : map.getSize(),
			maxZoom : 16,
			duration : 1000,
		});
	...
	case 3:
		...
		map.getView().fit([ arrX[0], arrY[0], arrX[2], arrY[2] ], {
			size : map.getSize(),
			maxZoom : 16,
			duration : 1000,
		});
	...
	case 4:
		...
		getWMS();
		getWFS();
		map.getView().fit([ arrX[0], arrY[0], arrX[3], arrY[3] ], {
			size : map.getSize(),
			maxZoom : 16,
			duration : 1000,
		});
